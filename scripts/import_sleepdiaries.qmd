...
```{r}
sleepdiary <- 
  read_csv2(files[c(2:10)], show_col_types = FALSE) |> drop_na(redcap_repeat_instance)
sleepdiary2 <- 
  read_csv2(files[1], show_col_types = FALSE) |> drop_na(redcap_repeat_instance)
sleepdiary <- 
sleepdiary2 |> bind_rows(sleepdiary)
...
sleepdiary |> 
  tbl_summary(include = -c(comments, record_id),
              ...
              type = list(awakenings ~ "continuous", awake_duration ~ "continuous"),
