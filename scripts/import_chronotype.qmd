...
#collect files
files <- filefinder("chronotype",individual = TRUE, negate = "lookup|morning")
#import files
data <- read_csv2(files, show_col_types = FALSE) |> 
      mutate(record_id = parse_number(record_id),
           record_id = sprintf("TUM_S%03d", record_id))
#check column types
...
1 participant provided an unlikely bedtime before noon (and have not indicated shiftwork) - these participants will be treated as if their bedtime is 12 hours later (problem with am/pm)

```{r}
Id_to_shift <- c("TUM_S010")

data <-
  data %>% 
  mutate(across(contains(c("bedtime_work")), 
                \(x) case_when(
                  record_id %in% Id_to_shift ~ (x + hms::hms(hours = 12)) %>% hms::as_hms(),
                  .default = x))
         )
```
...
data <- 
  ...
  select(-contains("_scored"), -morning_eveningness_questionnaire_meq_complete)
