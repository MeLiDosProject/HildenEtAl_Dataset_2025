...
#collect files
files <- filefinder("mHLEA_app", continuous = TRUE, individual = TRUE)
#import files
...
#collect files
files <- 
  filefinder("mHLEA_excel", continuous = TRUE, individual = TRUE)
...
```{r}
data$Date[62] <- data$Date[61]-ddays(1)
data$Date[46] <- data$Date[46]-ddays(1)
data$Date[6] <- data$Date[6]-ddays(1)
data <- 
data_paper |> 
...
data_llm <- 
  ...
  mutate(start = force_tz(start, tzs[[site]]))
#add output to original
data <- data |> left_join(data_llm, by = c("record_id", "start", "activity_desc"))
data <-
  data |> 
